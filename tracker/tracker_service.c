...
static void *accept_thread_entrance(void* arg)
{
	...
	while (g_continue_flag)
	{
		...
		//logDebug("file: "__FILE__", line: %d, write incomesock to work thread, incomesock: %d =========================", __LINE__, incomesock);
		//连接建立，将客户端fd通过管道发送给work线程，由work线程处理客户端请求（客户端连接可能是storage，也可能是业务客户端）
...
void tracker_accept_loop(int* server_sock, int count)
{
	if(count < 1)
	{
		logError("file: "__FILE__", line: %d, " \
		"No server_sock accept client ",
		__LINE__);
		exit(-1);
	}

	//第一个sock在主线程中阻塞accept，后续的服务端sock创建新的线程accept创建连接
	if (count > 1)
	{
		pthread_t tid;
		pthread_attr_t thread_attr;
...
static int tracker_deal_parameter_req(struct fast_task_info *pTask)
{
	...
	sprintf(pTask->data + sizeof(TrackerHeader), \
		...
		"reprot_stroage_client_connect=%d\n"
    ...
    g_reprot_stroage_client_connect,\
...
//	if (splitEx(pIpAndPort, ':', ipAndPort, 2) != 2)
	if((p = strrchr(pIpAndPort, ':')) == NULL)
	{
		...
		//例如fe80::d294:66ff:fe43:3fee:32122，操作后变为fe80::d294:66ff:fe43:3fee\032122，ipAndPort[0]取\0前面，ipAndPort[1]取后面
		ipAndPort[0] = pIpAndPort;
		size_t ip_len = p - pIpAndPort;
		pIpAndPort[ip_len] = '\0';	//以'\0'截断pIpAndPort
		ipAndPort[1] = p + 1;
	}
	/************ modify by xiaods end ***********/
...
static int tracker_deal_storage_join(struct fast_task_info *pTask)
{
	...
	logDebug("yb_test:file: "__FILE__", line: %d, " \
			"run deal join storage ,cli ip:%s", \
			__LINE__,pTask->client_ip);
	...
	//tracker解析storage的请求，得到storage中的tracker集群地址，并更新到g_tracker_servers中
...
/************ modify by xiaods start ***********/
		//若是ipv6地址，需要使用strrchr来解析地址
		if ((pSeperator=strrchr(p, ':')) == NULL)
		{
			logError("file: "__FILE__", line: %d, " \
...
//		tracker_ip, IP_ADDRESS_SIZE);
	...
	int port;
	anetSockName(pTask->event.fd,tracker_ip,IP_ADDRESS_SIZE, &port);
/************ modify by xiaods end ***********/

	insert_into_local_host_ip(tracker_ip);

	//将新加入的storage信息保存到文件中
...
//add by 20170821 start
static int tracker_deal_storage_connect_report(struct fast_task_info *pTask)
{
	int nPkgLen;
	int status;
	char *recvBuff = NULL;
	...
	do
	{
		...
		recvBuff = pTask->data + sizeof(TrackerHeader);
		nClientNum = nPkgLen/sizeof(FDFSClientInfo);

		if( nPkgLen != 0 && nPkgLen%sizeof(FDFSClientInfo) != 0)
		{
			logError("file: "__FILE__", line: %d, " \
					"cmd=%d, client ip: %s, package size " \
					PKG_LEN_PRINTF_FORMAT" is not correct, " \
					"expect length: 0 or %d", __LINE__, \
					TRACKER_PROTO_CMD_STORAGE_CONNECTION, \
					pTask->client_ip, nPkgLen,nClientNum*sizeof(FDFSClientInfo));
				status = EINVAL;
				break;
		}
		...
		sprintf(szID,"%s:%d",pClientInfo->pStorage->ip_addr,\
				pClientInfo->pStorage->storage_port);
		...
		{
			logError("file: "__FILE__", line: %d, " \
						"cmd=%d, client ip: %s, not fund storage "
						"%s is not correct, ", __LINE__, \
						TRACKER_PROTO_CMD_STORAGE_CONNECTION, \
						pClientInfo->pStorage->ip_addr,szID);
					status = EINVAL;
			break;
		}
		...
		if(nPkgLen == 0)
		{
			memset(pFound->clientInfo,0,sizeof(FDFSClientInfo)*pFound->cout);
			pFound->cout = 0;
			status = 0;
			break;
		}
		...
		//内存不足则动态增加
		while(pFound->alloc_cout < nClientNum )
		{
//			pFound->alloc_cout += ALLOC_IP_ASDDR_MEM_SIZE_ONCE;
//			pFound->clientInfo = (FDFSClientInfo*)realloc(\
//					pFound->clientInfo,sizeof(FDFSClientInfo)*pFound->alloc_cout);
//			memset(pFound->clientInfo + pFound->cout,0,\
//					sizeof(FDFSClientInfo)*ALLOC_IP_ASDDR_MEM_SIZE_ONCE);
			pFound->alloc_cout += ALLOC_IP_ASDDR_MEM_SIZE_ONCE;
			FDFSClientInfo *new_client;
			FDFSClientInfo *old_client;
			new_client = (FDFSClientInfo*)malloc(\
					sizeof(FDFSClientInfo)*pFound->alloc_cout);
			//memset(new_client,0,sizeof(FDFSClientInfo)*pFound->alloc_cout);

			old_client = pFound->clientInfo;
			pFound->clientInfo = new_client;
		...
		//更新该storage的客户端信息
		recvBuff = pTask->data + sizeof(TrackerHeader);
		memset(pFound->clientInfo,0,sizeof(FDFSClientInfo)*pFound->alloc_cout);
		pFound->cout = 0;

		memcpy(pFound->clientInfo,recvBuff,sizeof(FDFSClientInfo)*nClientNum);
		pFound->cout = nClientNum;
		status = 0;
...
static int tracker_deal_server_list_all_clients(struct fast_task_info *pTask)
{
	FDFSClientInfo *pDest;
	char storage_id[IP_ADDRESS_PORT_SIZE];

	FDFSClient *pFound = NULL;

	if (pTask->length - sizeof(TrackerHeader) != IP_ADDRESS_PORT_SIZE)
	{
		logError("file: "__FILE__", line: %d, " \
			"cmd=%d, client ip: %s, package size " \
			PKG_LEN_PRINTF_FORMAT" is not correct, " \
			"expect length: 0", __LINE__, \
			TRACKER_PROTO_CMD_SERVER_LIST_ALL_CLIENTS, \
			pTask->client_ip, pTask->length - \
			(int)sizeof(TrackerHeader));
		pTask->length = sizeof(TrackerHeader);
		return EINVAL;
	...
	memset(storage_id,0,IP_ADDRESS_PORT_SIZE);
	memcpy(storage_id,pTask->data + sizeof(TrackerHeader),IP_ADDRESS_PORT_SIZE);

	pFound = tracker_mem_get_cliStorage(&g_cli_Manager,storage_id);
	if(pFound ==  NULL)
	{
		logError("file: "__FILE__", line: %d, " \
			"client ip: %s, invalid storage: %s", \
			__LINE__, pTask->client_ip, storage_id);
		pTask->length = sizeof(TrackerHeader);
		return ENOENT;
	}
	...
	pDest = (FDFSClientInfo *)(pTask->data + sizeof(TrackerHeader));
	memcpy(pDest,pFound->clientInfo,sizeof(FDFSClientInfo)*pFound->cout);

	pTask->length = sizeof(TrackerHeader) + sizeof(FDFSClientInfo)*pFound->cout;
...
//	logDebug("file: "__FILE__", line: %d, tracker_deal_task, cmd: %d ========================", __LINE__, pHeader->cmd);
	switch(pHeader->cmd)
	{
		...
		case TRACKER_PROTO_CMD_TRACKER_NOTIFY_RESELECT_LEADER:
			...
			break;
		//add by yangb 20170816:start
		case TRACKER_PROTO_CMD_STORAGE_CONNECTION:
			result = tracker_deal_storage_connect_report(pTask);
			break;
		case TRACKER_PROTO_CMD_SERVER_LIST_ALL_CLIENTS:
			result = tracker_deal_server_list_all_clients(pTask);
			break;
		//end
		default:
			logError("file: "__FILE__", line: %d, "  \
	...
	if(result != 0)
		logError("file: "__FILE__", line: %d, tracker_deal_task end, cmd: %d, status: %d ", __LINE__, pHeader->cmd, result);
	...
	//logDebug("cmd: %d\n, response pkg_len: %d\n", cmd, pTask->length - sizeof(TrackerHeader));
