		...
		char **bindaddr, int *bindaddr_count)
{
	...
	do
	{
		...
		char bind_addr[IP_ADDRESS_SIZE] = { 0 };
		if (pBindAddr == NULL)
		{
			bind_addr[0] = '\0';
		}
		...
		{
			int addr_size = strlen(pBindAddr);
			strncpy(bind_addr, pBindAddr, addr_size);
//			snprintf(bind_addr, addr_size, "%s", pBindAddr);
			bind_addr[addr_size] = '\0';
		}

		if(pBindAddr == NULL)
			*bindaddr_count = 0;
		else{
			char *p = strtok(bind_addr, ",");
			while(p != NULL){
				size_t l = strlen(p) + 1;
				bindaddr[*bindaddr_count] = (char *)malloc(l);
				memcpy(bindaddr[*bindaddr_count], p, l);
				logInfo("[%d] p: %s, ptr: %s\n", *bindaddr_count, p, bindaddr[*bindaddr_count]);
				(*bindaddr_count)++;
				p = strtok(NULL, ",");
			}
		}
		...
		//读取allow_hosts配置项，能够连接到此 tracker server 的ip范围（对全部类型的连接都有影响，包含客户端，storage server）
		//默认设置为*，表示所有ip都能接入
		...
		//add by yb
		g_reprot_stroage_client_connect = iniGetBoolValue(NULL, \
						"reprot_stroage_client_connect", \
						&iniContext, true);
