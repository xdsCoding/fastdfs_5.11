...
//add by yb
static int tracker_mem_add_cliStorage(FDFSClientManager *pClientManger,\
		const char *id,const bool bNeedLock);

static int tracker_mem_destroy_cliManager(FDFSClientManager *pClientManager);
...
//add by yb start
static int tracker_mem_realloc_cliStorages(FDFSClientManager *pClientManger,\
		const bool bNeedSleep)
{
	FDFSClient **new_storages;
	FDFSClient **old_storages;
	FDFSClient **new_sort_storages;
	FDFSClient **old_sort_storage;
	FDFSClient **pp;
	FDFSClient **ppEnd;
	int new_size;
	new_size = pClientManger->alloc_cout + TRACKER_MEM_ALLOC_ONCE;
	new_storages = (FDFSClient**)malloc(sizeof(FDFSClient*)*new_size);
	if(new_storages ==  NULL)
	{
		logError("file: "__FILE__", line: %d, " \
					"malloc %d bytes fail", \
					__LINE__, (int)sizeof(FDFSClient) * new_size);
				return errno != 0 ? errno : ENOMEM;
	}

	memset(new_storages,0,sizeof(FDFSClient*)*new_size);
	ppEnd = new_storages + new_size;
	for(pp = new_storages + pClientManger->cout;pp<ppEnd;pp++)
	{
		*pp = (FDFSClient*)malloc(sizeof(FDFSClient));
		memset(*pp,0,sizeof(FDFSClient));
		if(*pp == NULL)
		{
			logError("file: "__FILE__", line: %d, " \
						"malloc %d bytes fail", \
						__LINE__, (int)sizeof(FDFSClient));
					return errno != 0 ? errno : ENOMEM;
			//释放内存
		}
	...
	memcpy(new_storages,pClientManger->client,sizeof(FDFSClient*)*pClientManger->cout);

	new_sort_storages = (FDFSClient**)malloc(sizeof(FDFSClient*)*new_size);
	if(new_sort_storages ==  NULL)
	{
		logError("file: "__FILE__", line: %d, " \
					"malloc %d bytes fail", \
					__LINE__, (int)sizeof(FDFSClient*) * new_size);
	...
	memset(new_sort_storages,0,sizeof(FDFSClient*)*new_size);
	memcpy(new_sort_storages,pClientManger->sort_client,\
			sizeof(FDFSClient*)*pClientManger->cout);

	old_storages = pClientManger->client;
	old_sort_storage = pClientManger->sort_client;
	pClientManger->client = new_storages;
	pClientManger->sort_client = new_sort_storages;
	pClientManger->alloc_cout = new_size;
	...
	if(old_storages != NULL)
	{
		free(old_storages);
	}
	if(old_sort_storage != NULL)
	{
		free(old_sort_storage);
	}
...
static int tracker_load_data(FDFSGroups *pGroups,FDFSClientManager *pClientManger)
{
...
static int tracker_mem_init_groups(FDFSGroups *pGroups,FDFSClientManager *pClientManager)
{
	int result;
	FDFSGroupInfo **ppGroup;
	FDFSGroupInfo **ppGroupEnd;
	...
	FDFSClient **pp;
	FDFSClient **ppEnd;
	pClientManager->alloc_cout = TRACKER_MEM_ALLOC_ONCE;
	pClientManager->cout = 0;
	pClientManager->client = (FDFSClient**)malloc(sizeof(FDFSClient*)*pClientManager->alloc_cout);
	if (pClientManager->client == NULL)
	{
		logCrit("file: "__FILE__", line: %d, " \
			"malloc %d bytes fail!", __LINE__, \
			(int)sizeof(FDFSClient *) * pClientManager->alloc_cout);
		return errno != 0 ? errno : ENOMEM;
	}

	memset(pClientManager->client,0,sizeof(FDFSClient*)*pClientManager->alloc_cout);

	ppEnd = pClientManager->client + pClientManager->alloc_cout;
	for(pp =  pClientManager->client;pp<ppEnd;pp++)
	{
		*pp = (FDFSClient*)malloc(sizeof(FDFSClient));
		if (*pp == NULL)
		{
			//释放pClientManager->client
			if(pClientManager->client != NULL)
			{
				free(pClientManager->client);
				pClientManager->client = NULL;
			}
	...
	pClientManager->sort_client = (FDFSClient**)\
			malloc(sizeof(FDFSClient*)*pClientManager->alloc_cout);

	memset(pClientManager->sort_client,0,sizeof(FDFSClient*)*pClientManager->alloc_cout);

	if (pClientManager->sort_client == NULL)
	{
		FDFSClient **pp;
		FDFSClient **ppEnd;
...
//		return result;
//	}
	if ((result=tracker_load_data(pGroups,pClientManager)) != 0)
	{
		return result;
	}
...
int tracker_mem_init()
{
	...
	return tracker_mem_init_groups(&g_groups,&g_cli_Manager);
    //end
}
...
//add by yb start
static int tracker_mem_init_cliStorage(FDFSClient *pSrc)
{

	pSrc->alloc_cout = ALLOC_IP_ASDDR_MEM_SIZE_ONCE;
	pSrc->cout = 0;
	pSrc->clientInfo = (FDFSClientInfo*)malloc(sizeof(FDFSClientInfo)*pSrc->alloc_cout);
	if(pSrc == NULL)
	{
		logError("file: "__FILE__", line: %d, " \
					"malloc %d bytes fail", __LINE__, \
					(int)sizeof(FDFSClientInfo) * pSrc->alloc_cout);
				return errno != 0 ? errno : ENOMEM;
	}

	memset(pSrc->clientInfo,0,sizeof(FDFSClientInfo)*pSrc->alloc_cout);

	return 0;

}

static void tracker_mem_insert_into_sorted_cliServers( FDFSClientManager *pClientManager, \
		FDFSClient *pTargetServer)
{
	FDFSClient **ppServer;
	FDFSClient **ppEnd;

	ppEnd = pClientManager->sort_client + pClientManager->cout;
	for (ppServer=ppEnd;ppServer>pClientManager->sort_client; ppServer--)
	{
...
static int tracker_mem_destroy_cliManager(FDFSClientManager *pClientManager)
{
	...
	else
	{
		...
		if(pClientManager->sort_client != NULL)
		{
			free(pClientManager->sort_client);
		    pClientManager->sort_client = NULL;
		}
...
int tracker_mem_cmp_by_client_id(const void *p1, const void *p2)
{
	return strcmp((*((FDFSClient **)p1))->id,\
			(*((FDFSClient **)p2))->id);
}
//end
...
//add by yb
FDFSClient* tracker_mem_get_cliStorage(FDFSClientManager *pClientManger,const char *id)
{
	FDFSClient target_cli;
	FDFSClient **pCliStorage;
	FDFSClient *pTarget_cli;
	memset(&target_cli,0,sizeof(target_cli));
	strcpy(target_cli.id,id);
	pTarget_cli = &target_cli;
	pCliStorage = (FDFSClient**)bsearch(&pTarget_cli,\
			pClientManger->sort_client,pClientManger->cout,sizeof(FDFSClient*),\
			tracker_mem_cmp_by_client_id);
		...
		const char *id,const bool bNeedLock)
{
    ...
    do
    {
		...
		psc = tracker_mem_get_cliStorage(pClientManger,id);
		if(psc != NULL)
		{
			//test
			logDebug("yb_test:file: "__FILE__", line: %d, " \
					"exist cliStorage %s,current Stor=%d\n ",\
					__LINE__,psc->id,pClientManger->cout);
			break;
		}

		if(pClientManger->cout >= pClientManger->alloc_cout)
		{
			result = tracker_mem_realloc_cliStorages(pClientManger,true);
			if(result != 0)
			{
				return result;
			}
		...
		psc = *(pClientManger->client+pClientManger->cout);
		result = tracker_mem_init_cliStorage(psc);
		if (result != 0)
		{
			break;
		}
		...
		psc = *(pClientManger->client + pClientManger->cout);
		strcpy(psc->id,id);
		tracker_mem_insert_into_sorted_cliServers(pClientManger,psc);
		pClientManger->cout++;

    }while(0);

	if (bNeedLock && (result=pthread_mutex_unlock(&mem_thread_lock)) != 0)
	{
		logError("file: "__FILE__", line: %d, " \
			"call pthread_mutex_lock fail, " \
			"errno: %d, error info: %s", \
		 ...
		 TrackerRunningStatus *pRunningStatus)
{
	...
	//add by yb
	FDFSClientManager newClientManager;
	FDFSClientManager tmpClientManager;
	memset(&newClientManager,0,sizeof(FDFSClientManager));
	memset(&tmpClientManager,0,sizeof(FDFSClientManager));

	//end
	if ((result=tracker_mem_init_groups(&newGroups,&newClientManager)) != 0)
	{
 		tracker_mem_destroy_groups(&newGroups, false);
 		//释放内存
 		//tracker_mem_destroy_cliManager(&newClientManager);
		return result;
	}

	//add by by
	memcpy(&tmpClientManager,&g_cli_Manager,sizeof(FDFSClientManager));
	memcpy(&g_cli_Manager,&newClientManager,sizeof(FDFSClientManager));
	tracker_mem_destroy_cliManager(&tmpClientManager);
	//end
		...
		const bool bNeedSleep)
{
	...
	//add by yb
	char szId[IP_ADDRESS_PORT_SIZE] = {0};
	sprintf(szId,"%s:%d",ip_addr,pJoinBody->storage_port);
	if((result = tracker_mem_add_cliStorage(&g_cli_Manager,szId,true)) != 0)
	{
		return result;
	}
	//end
	...
	do
	{
		...
		for (pServer=briefServers; pServer<pEnd; pServer++)
		{
			...
			else
			{
				...
				//add by yb
				char szId[IP_ADDRESS_PORT_SIZE] = {0};
	            sprintf(szId,"%s:%s",pServer->ip_addr,pServer->port);
	            if((result = tracker_mem_add_cliStorage(&g_cli_Manager,szId,true)) != 0)
	            {
				    pStorageServer->status = pServer->status;
...
//add by yb 20170825
int tracker_mem_remove_storage_clients(char *storage_ip,int storage_port)
{
	int result;
	FDFSClient *pFound;
	char szID[IP_ADDRESS_PORT_SIZE] = {0};
	...
	sprintf(szID,"%s:%d",storage_ip,storage_port);

	pFound = tracker_mem_get_cliStorage(&g_cli_Manager,szID);

	if(pFound != NULL)
	{
		//下线移除连接storage的客户端信息
		memset(pFound->clientInfo,0,sizeof(FDFSClient)*pFound->cout);
		pFound->cout = 0;
	}
