...
void task_finish_clean_up(struct fast_task_info *pTask)
{
	...
	if (pClientInfo->pGroup != NULL)
	{
		if (pClientInfo->pStorage != NULL)
		{
			...
			{
				tracker_mem_remove_storage_clients(\
...
/************ modify by xiaods end ***********/		
		...
		{
			//g_allow_ip_count与配置文件中的allow_hosts有关，默认值为空，表示所有ip都可以连过来
...
static void client_sock_read(int sock, short event, void *arg)
{
	...
	while (1)
	{
		...
		//数据可能需要分多次发送过来，需要获取sizeof(TrackerHeader)数据，offset用于记录当前偏移量，判断数据包是否完整
		...
		if (bytes < 0)
		{
			if (errno == EAGAIN || errno == EWOULDBLOCK)
			{
				//非阻塞socket,socket接收缓冲区中没有数据时会立刻返回，错误码为EWOULDBLOCK
			}
			else if (errno == EINTR)
			{
				continue;
