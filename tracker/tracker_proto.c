...
static int fdfs_do_parameter_req(ConnectionInfo *pTrackerServer, \
	char *buff, const int buff_size)
{
	//发送请求给tracker，g_fdfs_network_timeout为网络请求超时时间
	//storgae和tracker通信时，基本都是这种模式，非阻塞socket，tcpsenddata_nb发送请求数据后，立刻调用fdfs_recv_response接收响应做相应操作
...
/************ modify by xiaods start ***********/
		...
		for (i=0; i < 3; i++)
		{
			logDebug("file: "__FILE__", line: %d, connect to tracker, index: %d =========================", __LINE__, i);
			...
			{
				//连接tracker
				//成功进行后续发送TRACKER_PROTO_CMD_STORAGE_PARAMETER_REQ请求
				//失败重试sock置为-1，最多三次
				break;
			}
		...
		pTServer->sock = connectserverbyip_nb_v6(pTServer->ip_addr, pTServer->port, g_fdfs_connect_timeout, client_bind_addr, bind_addr);
