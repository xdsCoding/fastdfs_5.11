...
{
	//traker选主，新加入的storgae会将自身配置的tracker地址信息发送过来，tracker解析并更新到g_tracker_servers得到最新的tracker集群地址
	//根据g_tracker_servers中的地址信息，进行互连（心跳？），得到各个tracke服务的状态，进行选主
...
static int relationship_ping_leader()
{
	...
	if (pTrackerServer->sock < 0)
	{
		...
		{
			logError("file: "__FILE__", line: %d, " \
						"connect to leader %s:%d failed, result: %d", __LINE__, pTrackerServer->ip_addr, pTrackerServer->port, result);
			return result;
		}
	}
