...
//add by yb 20170826 start
int tracker_list_clients(ConnectionInfo *pTrackerServer, \
		const char* client_storage_id,
		FDFSClientInfo *clients_infos, \
		const int max_clients,
		int *client_count)
{

	char out_buff[sizeof(TrackerHeader) + IP_ADDRESS_PORT_SIZE];
	bool new_connection;
	TrackerHeader *pHeader;
	ConnectionInfo *conn;
	int result;
	char *pInBuff;
	FDFSClientInfo pSrc[FDFS_MAX_CLIENTS_EACH_STROAGE];
	int64_t in_bytes;
	...
	memset(out_buff, 0, sizeof(out_buff));
	pHeader = (TrackerHeader *)out_buff;
	memcpy(out_buff + sizeof(TrackerHeader),client_storage_id,strlen(client_storage_id));

	long2buff(IP_ADDRESS_PORT_SIZE,pHeader->pkg_len);
	pHeader->cmd = TRACKER_PROTO_CMD_SERVER_LIST_ALL_CLIENTS;
	if ((result=tcpsenddata_nb(conn->sock, out_buff, \
		sizeof(TrackerHeader) + IP_ADDRESS_PORT_SIZE, \
		g_fdfs_network_timeout)) != 0)
	{
		logError("file: "__FILE__", line: %d, " \
				"send data to tracker server %s:%d fail, " \
	...
	if (in_bytes % sizeof(FDFSClientInfo) != 0)
	{
		logError("file: "__FILE__", line: %d, " \
			"tracker server %s:%d response data " \
			"length: %"PRId64" is invalid", \
	...
	*client_count = in_bytes / sizeof(FDFSClientInfo);
	if (*client_count > max_clients)
	{
		logError("file: "__FILE__", line: %d, " \
			...
			"max storage count: %d, expect count: %d", \
			__LINE__, pTrackerServer->ip_addr, \
			pTrackerServer->port, max_clients, *client_count);
		*client_count = 0;
		return ENOSPC;
	}

	memset(clients_infos, 0, sizeof(FDFSClientInfo) * max_clients);

    memcpy(clients_infos,&pSrc,sizeof(FDFSClientInfo)*(*client_count));
		...
		const char *group_name, const char *filename)
{
	...
	if(strchr(pStorageServer->ip_addr, ':') != NULL && strlen(g_network_card) > 0)
	{
		//ipv6类地址且需要带网卡
		strcat(pStorageServer->ip_addr, "%");
		strcat(pStorageServer->ip_addr, g_network_card);
	}
