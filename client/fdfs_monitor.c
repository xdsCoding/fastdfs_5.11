...
static int list_storage_clients(const char *client_storage_id)
{
	int result;
	int clients_count;
	FDFSClientInfo clients_info[FDFS_MAX_CLIENTS_EACH_STROAGE];
	FDFSClientInfo *p;
	FDFSClientInfo *pEnd;


	result = tracker_list_clients(pTrackerServer,client_storage_id,clients_info,\
			FDFS_MAX_CLIENTS_EACH_STROAGE,&clients_count);
	...
	p = clients_info;
	pEnd = clients_info + clients_count;
	for(p; p<pEnd; p++)
	{
		printf("\t\t\t%s:%d\n",p->ip_addr,p->port);
	}
...
static int list_storages(FDFSGroupStat *pGroupStat)
{
		...
		pStorage++)
	{
		...
		char client_storage_id[IP_ADDRESS_PORT_SIZE] = {0};
		sprintf(client_storage_id,"%s:%d",pStorage->ip_addr,pStorage->storage_port);
		printf("\t\tstorage active clients:\n",client_storage_id);
		list_storage_clients(client_storage_id);
		//end
	}
