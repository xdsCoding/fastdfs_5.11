		...
		const char *filename, char **ppTrackerServers)
{
	...
	for (ppSrc=ppTrackerServers; ppSrc<ppEnd; ppSrc++)
	{
		...
		if(strchr(szHost, ':') == NULL){
			//初步判断szHost不含:,是ipv4类型地址，利用getIpaddrByName进一步判断地址是否有效
			destServer.socket_domain = AF_INET;
			...
			//初步判断szHost含有:,是ipv6类型地址
			destServer.socket_domain = AF_INET6;
			if (getIpaddr6ByName(szHost, destServer.ip_addr, \
				sizeof(destServer.ip_addr)) == INADDR_NONE)
			{
				logError("file: "__FILE__", line: %d, " \
					"conf file \"%s\", " \
