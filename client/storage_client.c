...
Body	1954字节内容
out_buff[512]   10 + 1 + 8 + 6 = 25
上传1939大小文件过程：
0000 0000 00	 |			0			|		0000 0000		|		0000 00			|			0000 ... 0000
头部(10)			new_store_path(1)			file_bytes(8)		file_ext_name(6)	|			sendfile(1939)
pHeader											1939					.conf			|				文件内容
1954, 23, 0																				|
								tcpsenddata_nb											|			tcpsendfile
																						|
																					  p指针

storgae_nio.c -- client_sock_read 接收上传文件命令过程
recv_bytes = sizeof(TrackerHeader) - pTask->offset; (10)  先获取取头部pHeader()
pClientInfo->total_length = buff2long(((TrackerHeader *) \
	pTask->data)->pkg_len);		1954
**/
	...
	char *group_name, char *remote_filename)
{
	...
	*/
	...
	{
	pHeader = (TrackerHeader *)out_buff;
	p = out_buff + sizeof(TrackerHeader);		// 10
	if (bUploadSlave)
	{
	...
	{
		*p++ = (char)new_store_path;	//out_buff[10]	11
	}

	long2buff(file_size, p);			//file_size 1939
	p += FDFS_PROTO_PKG_LEN_SIZE;		// 19
	...
	p += FDFS_FILE_EXT_NAME_MAX_LEN;		//25
	...
	long2buff((p - out_buff) + file_size - sizeof(TrackerHeader), \
		pHeader->pkg_len);			//25 + 1939 - 10 = 1954
	pHeader->cmd = cmd;
	pHeader->status = 0;
