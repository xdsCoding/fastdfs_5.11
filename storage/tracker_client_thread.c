...
static int tracker_storage_client_connect_report(ConnectionInfo *pTrackerServer, \
		int *stat_storage_client_connect_chg_count, bool *bServerPortChanged);
...
static void *tracker_report_thread_entrance(void *arg)
{
	...
	//add by yb
	int stat_storage_client_connect_chg_count;
	//end
...
/************ modify by xiaods start ***********/
		...
		pTrackerServer->sock = connectserverbyip_nb_v6(pTrackerServer->ip_addr, pTrackerServer->port, g_fdfs_connect_timeout, g_client_bind_addr, g_bind_addr);
		if(pTrackerServer->sock < 0)
		{
			nContinuousFail++;
			if (g_continue_flag)
		...
		int port;
		anetSockName(pTrackerServer->sock,tracker_client_ip,IP_ADDRESS_SIZE, &port);
...
/************ modify by xiaods end ***********/
		...
		//add by yb
		stat_storage_client_connect_chg_count = 0;
		//end
		...
		while (g_continue_flag)
		{
					...
					g_heart_beat_interval)
			{
				...
				//add by yb 20170816:report storage connect client info.protocal:TRACKER_PROTO_CMD_STORAGE_CONNECTION
				if(g_reprot_stroage_client_connect &&\
				  stat_storage_client_connect_chg_count != g_stat_storage_client_connect_chg_count&& \
					tracker_storage_client_connect_report(pTrackerServer,\
					&stat_storage_client_connect_chg_count,\
					&bServerPortChanged) != 0)
				{
					break;
				}
	...
	bool *bServerPortChanged)
{
	...
	logDebug("resp server count=%d", server_count);
	{
		int i;
			...
			const int tracker_index, const bool sync_old_done)
{
	...
	//storage加入集群时将tracker服务地址加到请求后面传给tracker，后续tracker会解析出地址
	//tracker_count = 0;
		...
		bool *bServerPortChanged)
{
	...
	//发送TRACKER_PROTO_CMD_STORAGE_SYNC_REPORT请求，从tracker上获取同组group的storage信息
	if((result=tcpsenddata_nb(pTrackerServer->sock, out_buff, \
...
static int tracker_storage_client_connect_report(ConnectionInfo *pTrackerServer, \
		int *stat_storage_client_connect_chg_count, bool *bServerPortChanged)
{
	char out_buff[sizeof(TrackerHeader) + sizeof(FDFSClientInfo)*g_stor_cli_manager.current_count];
	TrackerHeader *pHeader;
	FDFSClientInfo *send_clients;
	FDFSClientInfo **pp;
	...
	memset(out_buff, 0, sizeof(out_buff));
	pHeader = (TrackerHeader *)out_buff;
//	if (*stat_storage_client_connect_chg_count != g_stat_storage_client_connect_chg_count)
//	{
		send_clients =(FDFSClientInfo*)(out_buff + sizeof(TrackerHeader));

		if(g_stor_cli_manager.current_count > 0)
		{
			ppEnd = g_stor_cli_manager.clients + g_stor_cli_manager.current_count;
			for(pp = g_stor_cli_manager.clients;pp<ppEnd;pp++)
			{
				memcpy(send_clients,*pp,sizeof(FDFSClientInfo));
				send_clients++;
			}
		}

		*stat_storage_client_connect_chg_count = g_stat_storage_client_connect_chg_count;
		body_len = sizeof(FDFSClientInfo)*g_stor_cli_manager.current_count;
//	}
//	else
//	{
//		body_len = 0;
//	}
	...
	logDebug("clients connected:alloc_count: %d, current_count: %d",
			g_stor_cli_manager.alloc_count,g_stor_cli_manager.current_count);

	long2buff(body_len, pHeader->pkg_len);
	pHeader->cmd = TRACKER_PROTO_CMD_STORAGE_CONNECTION;
