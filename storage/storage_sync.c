...
//	int i;
			...
			pGlobalServer++)
	{
		...
		pTServer->sock = connectserverbyip_nb_v6(pTServer->ip_addr, pTServer->port, g_fdfs_connect_timeout, g_client_bind_addr, g_bind_addr);
		logDebug("file: "__FILE__", line: %d, connect to tracker, sock: %d =========================", __LINE__, pTServer->sock);
		if (pTServer->sock < 0)
		{
			logError("file: "__FILE__", line: %d, " \
...
//	int conn_ret;
	...
	do
	{
		...
		int port;
		anetSockName(pTServer->sock,tracker_client_ip,IP_ADDRESS_SIZE, &port);
...
//		previousCode = 0;
			...
			pStorage->status != FDFS_STORAGE_STATUS_NONE)
		{
			...
			char addr[IP_ADDRESS_SIZE] = {0};
			if(strlen(g_network_card) > 0 && strchr(pStorage->ip_addr, ':') != NULL)
			{
				//若为Ipv6地址，且需要带上网卡信息，则在ipv6地址后面加上%网卡
				sprintf(addr, "%s%%%s", pStorage->ip_addr, g_network_card);
			}
			else
			{
				//ipv4地址，或者不需带网卡信息
				strcpy(addr, pStorage->ip_addr);
			}

			storage_server.sock = connectserverbyip_nb_v6(addr, g_server_port, g_fdfs_connect_timeout, g_client_bind_addr, g_bind_addr);
			if(storage_server.sock > 0)
			{
...
//			local_ip_addr, IP_ADDRESS_SIZE);
		int port;
		anetSockName(storage_server.sock, local_ip_addr, sizeof(local_ip_addr), &port);
