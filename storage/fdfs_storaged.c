...
int listenToPort()
{
	...
	int result = 0;
	int j;
	if (g_bindaddr_count == 0) g_bindaddr[0] = NULL;
	for (j = 0; j < g_bindaddr_count || j == 0; j++) {
		if (g_bindaddr[j] == NULL) {
			int unsupported = 0;
			/* Bind * for both IPv6 and IPv4, we enter here only if
			 * server.bindaddr_count == 0. */
			g_ipfd[g_ipfd_count] = socketServer6(NULL, g_server_port, &result);
			if (g_ipfd[g_ipfd_count] != -1) {
				if ((result=tcpsetserveropt(g_ipfd[g_ipfd_count], g_fdfs_network_timeout)) != 0)
				{
			...
			if (g_ipfd_count == 1 || unsupported) {
				/* Bind the IPv4 address as well. */
				g_ipfd[g_ipfd_count] = socketServer(NULL, g_server_port, &result);
				if (g_ipfd[g_ipfd_count] != -1) {
					if ((result=tcpsetserveropt(g_ipfd[g_ipfd_count], g_fdfs_network_timeout)) != 0)
					{
						logError("IPv4 tcpsetserveropt error");
			...
			if (g_ipfd_count + unsupported == 2) break;
		}
		else if (strchr(g_bindaddr[j],':')) {
			logInfo("bindaddr[%d]: %s", j, g_bindaddr[j]);
			/* Bind IPv6 address. */
			g_ipfd[g_ipfd_count] = socketServer6(g_bindaddr[j], g_server_port, &result);
		}
		else {
			logInfo("bindaddr[%d]: %s", j, g_bindaddr[j]);
			/* Bind IPv4 address. */
			g_ipfd[g_ipfd_count] = socketServer(g_bindaddr[j], g_server_port, &result);
		}

		if (g_ipfd[g_ipfd_count] == -1) {
			logError("Creating Server TCP listening socket %s:%d",
				g_bindaddr[j] ? g_bindaddr[j] : "*", g_server_port);
			return -1;
		}
		if ((result=tcpsetserveropt(g_ipfd[g_ipfd_count], g_fdfs_network_timeout)) != 0)
		{
...
*/
	...
	storage_accept_loop(g_ipfd, g_ipfd_count);
