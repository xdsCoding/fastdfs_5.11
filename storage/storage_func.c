...
//add by yb
#define STORAGE_MEM_ALLOC_ONCE	2
//end
...
{
	//从tracker处获取id
	//g_use_storage_id与tracker配置文件中的use_storage_id对应，若启用，则使用storage id代替ip地址来标识storage
	int ret = 0;

	if(strchr(g_tracker_client_ip, ':') == NULL){
		//ipv4类地址
		struct in_addr ip_addr;
		if ((ret =inet_pton(AF_INET, g_tracker_client_ip, &ip_addr)) == 1)
		{
			g_server_id_in_filename = ip_addr.s_addr;
		}
	}else{
		struct in6_addr ip6_addr;
		if ((ret =inet_pton(AF_INET6, g_tracker_client_ip, &ip6_addr)) == 1)
		{
			//g_use_storage_id不知道干什么的，若有用需把ipv6地址转换为in_addr_t
//			g_server_id_in_filename = ip6_addr.sin6_addr;
		...
		char *bind_addr, const int addr_size)
{
	...
	*/
	...
	do
	{
		...
		/************ modify by xiaods start ***********/
		//多ip地址解析
		char s_bind_addr[IP_ADDRESS_SIZE] = { 0 };
		if (pBindAddr == NULL)
		{
			s_bind_addr[0] = '\0';
		}
		...
		{
			int s_addr_size = strlen(pBindAddr);
			strncpy(s_bind_addr, pBindAddr, s_addr_size);
//			snprintf(bind_addr, addr_size, "%s", pBindAddr);
			s_bind_addr[s_addr_size] = '\0';
		}

		if(pBindAddr == NULL)
			g_bindaddr_count = 0;
		else{
			char *p = strtok(s_bind_addr, ",");
			while(p != NULL){
				size_t l = strlen(p) + 1;
				g_bindaddr[g_bindaddr_count] = (char *)malloc(l);
				memcpy(g_bindaddr[g_bindaddr_count], p, l);
				logInfo("[%d] p: %s, ptr: %s\n", g_bindaddr_count, p, g_bindaddr[g_bindaddr_count]);
				g_bindaddr_count++;
				p = strtok(NULL, ",");
			}
		}
		...
		//新增网卡配置项
		pNetworkCard = iniGetStrValue(NULL, "network_card", &iniContext);
		...
		{
			strncpy(g_network_card, pNetworkCard, sizeof(g_network_card));
		}
		...
		for (pServer=g_tracker_group.servers; pServer<pEnd; pServer++)
		{
			...
			if(strchr(pServer->ip_addr, ':') != NULL && strlen(g_network_card) > 0)
			{
				//ipv6类地址且需要网卡
				strcat(pServer->ip_addr, "%");
				strcat(pServer->ip_addr, g_network_card);
				logDebug("after strcat, server=%s:%d\n", pServer->ip_addr, pServer->port);
			}
		}
		/************ modify by xiaods end ***********/
		if (result != 0)
