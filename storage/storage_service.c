...
static int storage_read_from_file_ex(struct fast_task_info *pTask, \
	const int64_t file_offset, const int64_t download_bytes, \
	FileDealDoneCallback done_callback, \
	const int store_path_index);
		...
		struct fast_task_info *pTask, const int err_no)
{
	...
	logDebug("[storage_download_file_done_callback] pTask->length=%d, err_no=%d", pTask->length, err_no);
...
#define ALLOC_CONNECTIONS_ONCE 256
	...
	g_stor_cli_manager.alloc_count = ALLOC_IP_ASDDR_MEM_SIZE_ONCE;
	g_stor_cli_manager.current_count = 0;
	g_stor_cli_manager.clients = (FDFSClientInfo**)malloc(sizeof(FDFSClientInfo*)*g_stor_cli_manager.alloc_count);

	memset(g_stor_cli_manager.clients,0,sizeof(FDFSClientInfo*)*\
			g_stor_cli_manager.alloc_count);
	ppEnd = g_stor_cli_manager.clients + g_stor_cli_manager.alloc_count;
	for(pp =g_stor_cli_manager.clients;pp<ppEnd;pp++ )
	{
		*pp = (FDFSClientInfo*)malloc(sizeof(FDFSClientInfo));
		memset(*pp,0,sizeof(FDFSClientInfo));
	}

	g_stor_cli_manager.sorted_clients = (FDFSClientInfo**)malloc(sizeof(FDFSClientInfo*)*g_stor_cli_manager.alloc_count);
	memset(g_stor_cli_manager.sorted_clients,0,sizeof(FDFSClientInfo*)*\
			g_stor_cli_manager.alloc_count);
	//end
...
void storage_service_destroy()
{
	...
	pthread_mutex_destroy(&mem_thread_lock);
}
...
//add by yb
static void tracker_mem_insert_into_sorted_clients( FDFSStorCliManager *pStorCliManager, \
		FDFSClientInfo *pTargetServer)
{
	FDFSClientInfo **ppServer;
	FDFSClientInfo **ppEnd;

	ppEnd = pStorCliManager->sorted_clients + pStorCliManager->current_count;
	for (ppServer=ppEnd;ppServer>pStorCliManager->sorted_clients; ppServer--)
	{
		if (pTargetServer->port > (*(ppServer-1))->port)
		{
			break;
		}
		else
...
static int tracker_mem_realloc_clients(FDFSStorCliManager *pStorCliManager,const bool bNeedSleep)
{
	FDFSClientInfo **new_clients;
	FDFSClientInfo **new_sorted_clients;
	FDFSClientInfo **old_sorted_clients;

	FDFSClientInfo **old_clients;
	FDFSClientInfo **pp;
	FDFSClientInfo **ppEnd;

	int new_size;
	new_size = pStorCliManager->alloc_count + ALLOC_IP_ASDDR_MEM_SIZE_ONCE;
	new_clients = (FDFSClientInfo**)malloc(sizeof(FDFSClientInfo*)*new_size);
	if(new_clients == NULL)
	{
		logError("file: "__FILE__", line: %d, " \
				"malloc %d bytes fail, " \
				"errno: %d, error info: %s", \
				__LINE__, (int)sizeof(FDFSClientInfo*)*pStorCliManager->alloc_count, \
				errno, STRERROR(errno));
			return errno != 0 ? errno : ENOMEM;

	}

	memset(new_clients,0,sizeof(FDFSClientInfo*)*new_size);

	ppEnd = new_clients + new_size;
	for(pp = new_clients;pp<ppEnd;pp++)
	{
		*pp = (FDFSClientInfo*)malloc(sizeof(FDFSClientInfo));
		if(*pp == NULL)
		{
			...
			logError("file: "__FILE__", line: %d, " \
					"malloc %d bytes fail, " \
					"errno: %d, error info: %s", \
					__LINE__, (int)sizeof(FDFSClientInfo), errno, STRERROR(errno));
				return errno != 0 ? errno : ENOMEM;
		}
	...
	memcpy(new_clients,pStorCliManager->clients,\
			sizeof(FDFSClientInfo*)*pStorCliManager->current_count);

	new_sorted_clients = (FDFSClientInfo**)malloc(sizeof(FDFSClientInfo*)*new_size);
	if(new_sorted_clients == NULL)
	{
		free(new_clients);
		new_clients = NULL;
	...
	memset(new_sorted_clients,0,sizeof(FDFSClientInfo*)*new_size);
	memcpy(new_sorted_clients,pStorCliManager->sorted_clients,\
			sizeof(FDFSClientInfo*)*pStorCliManager->current_count);

	old_clients = pStorCliManager->clients;
	old_sorted_clients = pStorCliManager->sorted_clients;

	pStorCliManager->clients = new_clients;
	pStorCliManager->sorted_clients = new_sorted_clients;
	pStorCliManager->alloc_count  = new_size;
...
*/	
		int port;
		anetPeerToString(incomesock, szClientIp, IP_ADDRESS_SIZE, &port);
...
void storage_accept_loop(int* server_sock, int count)
{
	if(count < 1)
	{
		logError("file: "__FILE__", line: %d, " \
...
static void *work_thread_entrance(void* arg)
{
	...
	//�ܵ��Ŀ̶�fd��ӵ��¼�����б���
	//ͨ��libevent��عܵ��ɶ����Ƿ�ɶ������ܵ��ɶ�����ʹ��storage_recv_notify_read����
	//��ȡ�ܵ������Ϣ�����Թܵ���Ϣ���д��?
	...
	const int store_path_index)
{
	...
	/*pClientInfo->deal_func = dio_read_file;*/
	pClientInfo->deal_func = dio_read_file_ex;
	pClientInfo->clean_func = dio_read_finish_clean_up;
	pClientInfo->total_length = sizeof(TrackerHeader) + download_bytes;
	pClientInfo->total_offset = 0;

	pFileContext->op = FDFS_STORAGE_FILE_OP_SENDFILE;
	pFileContext->open_flags = O_RDONLY | g_extra_open_file_flags;
	pFileContext->offset = file_offset;
	pFileContext->start = file_offset;
	pFileContext->end = file_offset + download_bytes;
...
int storage_deal_task(struct fast_task_info *pTask)
{
	...
	logDebug("file: "__FILE__", line: %d, storage_deal_task end, client ip: %s, cmd: %d, status: %d ", __LINE__, pTask->client_ip, pHeader->cmd, result);
	if(result != 0 && result != STORAGE_STATUE_DEAL_FILE)
		logError("file: "__FILE__", line: %d, storage_deal_task end, client ip: %s, cmd: %d, status: %d ", __LINE__, pTask->client_ip, pHeader->cmd, result);
