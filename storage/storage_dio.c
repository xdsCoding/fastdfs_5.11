...
int dio_read_file(struct fast_task_info *pTask)
{
	...
	logInfo("###pFileContext->end=%d", pFileContext->end);

	if (pFileContext->offset < pFileContext->end)
	{
		logInfo("###pTask->length=%d", pTask->length);
		storage_nio_notify(pTask);  //notify nio to deal
	}
	else
	{
...
int dio_read_file_ex(struct fast_task_info *pTask)
{
	//dio_read_file_ex与dio_read_file不同，不再读取文件内容到pTask->data，交由nio线程直接sendfile文件内容到客户端
	StorageFileContext *pFileContext;
	pFileContext = &(((StorageClientInfo *)pTask->arg)->file_context);
	if (pFileContext->fd > 0)
	{
...
int dio_write_file(struct fast_task_info *pTask)
{
	...
	//add by guanyf 2017-07-13
	if (pFileContext->sync_flag == STORAGE_OP_TYPE_SOURCE_MODIFY_FILE || pFileContext->sync_flag == STORAGE_OP_TYPE_REPLICA_MODIFY_FILE)
	{
