...
bool is_local_host_ip(const char *client_ip)
{
	...
	for (p=g_local_host_ip_addrs; p<pEnd; p+=IP_ADDRESS_SIZE)
	{
		...
		//ipv6地址会带%网卡信息，需截取ipv6地址部分
		char *pi;
		char ip6_addr[IP_ADDRESS_SIZE];
		if((pi = strchr(client_ip, '%')) != NULL)
		{
			size_t len = pi - client_ip;
			strncpy(ip6_addr, client_ip, len);
			ip6_addr[len] = '\0';
//			logDebug("file: "__FILE__", line: %d, local host: %s, ip6_addr: %s =========================", __LINE__, p, ip6_addr);
			if (strcmp(ip6_addr, p) == 0)
			{
				return true;
			}
		}
