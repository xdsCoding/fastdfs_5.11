...
int setsockaddrbyip(const char *ip, const unsigned short port, struct sockaddr_in *addr,
...
int connectserverbyip(int sock, const char *server_ip, const unsigned short server_port)
{
    int result;
...
int connectserverbyip_nb_ex(int sock, const char *server_ip, \
		const unsigned short server_port, const int timeout, \
...
//add by yb 20170821 start
int getIpport(getnamefunc getname, int sock, int* port)
{
	struct sockaddr_in addr;
	socklen_t addrlen;
...
int connectserverbyip_nb_v6(const char *server_ip, const unsigned short server_port, const int timeout, bool client_bind, const char *bind_addr)
{
	int s=-1, rv, n, result;
...
//	int reuses = 0;
	...
	do
	{
		...
		if ((rv = getaddrinfo(server_ip,_port,&hints,&servinfo)) != 0) {
			//先按照默认ipv4类地址判断地址是否有效，无效设置为ipv6再进行判断
			 hints.ai_family = AF_INET6;
			 if ((rv = getaddrinfo(server_ip,_port,&hints,&servinfo)) != 0) {
				 logError("file: "__FILE__", line: %d, ip: %s, getaddrinfo error: %s =========================", __LINE__, server_ip, gai_strerror(rv));
				break;
			}
		}
		...
		for (p = servinfo; p != NULL; p = p->ai_next){
			...
			if (client_bind && *bind_addr != '\0')
			{
				int bound = 0;
				/* Using getaddrinfo saves us from self-determining IPv4 vs IPv6 */
				if ((rv = getaddrinfo(bind_addr, NULL, &hints, &bservinfo)) != 0) {
					logError("file: "__FILE__", line: %d, ip: %s, Can't get addr: %s =========================", __LINE__, bind_addr, gai_strerror(rv));
					goto error;
				}
				...
				if (!bound) {
					logError("file: "__FILE__", line: %d, ip: %s, Can't bind socket: %s =========================", __LINE__, server_ip, strerror(errno));
					goto error;
				}
			}
			...
			result = select(s+1, &rset, &wset, NULL, \
					g_fdfs_connect_timeout > 0 ? &tval : NULL);
		#else
			pollfds.fd = s;
			pollfds.events = POLLIN | POLLOUT;
			result = poll(&pollfds, 1, 1000 * timeout);
			...
			len = sizeof(result);
			if (getsockopt(s, SOL_SOCKET, SO_ERROR, &result, &len) < 0)
			{
				result = errno != 0 ? errno : EACCES;
				if(result == 0)
					goto end;
	...
	if(s < 0)
	{
		logError("file: "__FILE__", line: %d, Connect to %s:%d error: %s =========================", __LINE__, server_ip, server_port, strerror(result));
	}
	return s;
}
/************ add by xiaods end ***********/
...
/************ add by xiaods start ***********/
in_addr_t getIpaddr6ByName(const char *name, char *buff, const int bufferSize)
{
	...
	char ipv6addr[128];
	int ret;
	if((pSeperator=strchr(name, '%')) == NULL){
		ret = inet_pton(AF_INET6, name, &ip_addr);
	}else{
		int len = pSeperator - name;
		memcpy(ipv6addr, name, len);
		ipv6addr[len] = '\0';
		ret = inet_pton(AF_INET6, ipv6addr, &ip_addr);
	}
	logDebug("file: "__FILE__", line: %d, inet_pton ret=%d ========================", __LINE__, ret);
	if (ret == 1)
	{
		if (buff != NULL)
		{
...
int socketServer(const char *bind_ipaddr, const int port, int *err_no)
{
	...
	if ((result = getaddrinfo(bind_ipaddr,_port,&hints,&servinfo)) != 0) {
		logError("file: "__FILE__", line: %d, " \
				"getaddrinfo error: %s",
				__LINE__, gai_strerror(result));
		return -1;
	}
	for (p = servinfo; p != NULL; p = p->ai_next) {
		...
		if (af == AF_INET6 && anetV6Only(sock) == -1) goto error;
		if (anetSetReuseAddr(sock) == -1) goto error;
		if (anetListen(sock,p->ai_addr,p->ai_addrlen,1024) == -1) goto error;
		goto end;
	}
	if (p == NULL) {
		logError("file: "__FILE__", line: %d, " \
				"unable to bind socket, errno: %d",		\
				__LINE__, errno);
		goto error;
	}
...
int socketServer6(const char *bind_ipaddr, const int port, int *err_no)
{
	int sock = -1;
	int af = AF_INET6;
	int result;
...
//##############################add by dengw 20170113 start #######################
    struct pollfd pollfds;
    pollfds.fd = sock;
	pollfds.events = POLLOUT;

//##############################add by dengw 20170113 end #######################
...
//##############################add by dengw 20170113 start #######################
        ...
        if (result < 0)
        {
            ...
            printf("result < 0, errno:%d, errsrr: %s\n",errno, strerror(errno));
            close(fd);
            return errno != 0 ? errno : EINTR;
        }
        else if (result == 0)
...
//##############################add by dengw 20170113 end #######################
	...
	*total_send_bytes = file_bytes - remain_bytes;
    ...
    logError("tcpsenddatafunc send_func, errno:%d, errsrr: %s\n",errno, strerror(errno));
